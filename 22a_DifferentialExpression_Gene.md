...
#softlink all the bam files and their indices
for f in ../../38_Expression/01_AllExpressionDatasetsSeparate/*_sorted.bam; do ln -s $f;done
for f in ../../38_Expression/01_AllExpressionDatasetsSeparate/*_sorted.bam.bai; do ln -s $f;doneKJ3%$q       NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN5B.75-54KH

#genome with old names
ln -s ../../38_Expression/01_AllExpressionDatasetsSeparate/SCNgenome.fasta

#copy and rename the chromosomes to their old names, so the scaffolds agree with the bam files above
cp ../../49_RenameChromosomes/01_Transfer2Box/OrderedSCNGenePredictions.gff3 .
awk '{print $2"\t"$1}'  ../../49_RenameChromosomes/chromosome.map >NEW2OLDchromosome.map
ml maker
singularity shell "/opt/rit/singularity/images/maker/2.31.10_3.1/maker.simg"
map_data_ids NEW2OLDchromosome.map5599999000000000000000000000000000055.//////////////////////////////////////////0005 OrderedSCNGenePredictions.gff3
mv B PK.gff3 OldNamesOrderedSCNGenePredictions.gff3
```

### Run featurecounts on bam files with gffs that contain new names.
```
for f in *bam; do echo "ml subread; featureCounts -T 16 -M-p -t gene -s 2 -g ID -a OldNamesOrderedSCNGenePredictions.gff3  -o "${f%.*}"_GeneCounts.txt " $f"; done
```
...
less DeseqTable.txt |cut -f 1,11,12,13,14,15,16 |awk '{print $1,$2,$6,$7,$3,$4,$5}' |tr " " "\t"  >PA3vsMM10GlandDeseqTable.txt
 ...
 less PA3GlandvsMM10Gland |sed 's/,/\t/g' |awk 'NR<179 && NR>1 && $3<0{print $1"\t"$3}' >PA3GlandvsMM10Gland_PA3UP.tab
 less PA3GlandvsMM10Gland |sed 's/,/\t/g' |awk 'NR<179 && NR>1 && $3>0{print $1"\t"$3}' >PA3GlandvsMM10Gland_MM10UP.tab
...
FALSE  TRUE
 ...
 less pPA3CvsPA3Gland |sed 's/,/\t/g' |awk 'NR<3971 && NR>1 && $3<0{print $1"\t"$3}' >pPA3CvsPA3GlandListDown.tab
 less pPA3CvsPA3Gland |sed 's/,/\t/g' |awk 'NR<3971 && NR>1 && $3>0{print $1"\t"$3}' >pPA3CvsPA3GlandListUp.tab
...
FALSE  TRUE
 ...
 less ppPA3vspPA3C |sed 's/,/\t/g' |awk 'NR<269 && NR>1 && $3<0{print $1"\t"$3}' >ppPA3vspPA3CListDown.tab
 less ppPA3vspPA3C |sed 's/,/\t/g' |awk 'NR<269 && NR>1 && $3>0{print $1"\t"$3}' >ppPA3vspPA3CListUp.tab
```
...
10254    32
  ...
  less pPA3CvspPA3IC |sed 's/,/\t/g' |awk 'NR<33 && NR>1 && $3<0{print $1"\t"$3}' >pPA3CvspPA3ICListDown.tab
  less pPA3CvspPA3IC |sed 's/,/\t/g' |awk 'NR<33 && NR>1 && $3>0{print $1"\t"$3}' >pPA3CvspPA3ICListUp.tab
```
...
less DeseqTable.txt |cut -f 1,11,12,13,14,15,16 |awk '{print $1,$6,$7,$3,$4,$5}' |tr " " "\t"  >2PA3vs3MM10GlandDeseqTable.txt
 ...
 less 2PA3vs3MM10GlandGland |sed 's/,/\t/g' |awk 'NR<447 && NR>1 && $3<0{print $1"\t"$3}' >2PA3vs3MM10GlandListDown.tab
 less 2PA3vs3MM10GlandGland |sed 's/,/\t/g' |awk 'NR<447 && NR>1 && $3>0{print $1"\t"$3}' >2PA3vs3MM10GlandListUp.tab
...
awk 'NR==1' 2PA3vs2MM10GlandDeseqTable.txt |tr "\t" "\n"|awk '{if(NR==1){print $0} else if(NR<4) {print $0"\t0"} else {print $0"\t1"}}' >2PA3vs2MM10GlandConditionTable.txt
 ...
 less 2PA3vs2MM10GlandGland |sed 's/,/\t/g' |awk 'NR<1075 && NR>1 && $3<0{print $1"\t"$3}' >2PA3vs2MM10GlandListDown.tab
 less 2PA3vs2MM10GlandGland |sed 's/,/\t/g' |awk 'NR<1075 && NR>1 && $3>0{print $1"\t"$3}' >2PA3vs2MM10GlandListUp.tab
...
awk '$3=="gene"' ../../../38_Expression/01_AllExpressionDatasetsSeparate/mikado.loci.ancestral.gff3 |cut -f 9 |sed 's/ID=//g' |sed 's/;/\t/g' |awk '{print $1"\tN/A"}' >AllgenesNA
